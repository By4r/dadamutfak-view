...
- [ ] **Shell CSS alt-bloğu tam enjekte mi (görsel-only defekt yakalama):** Shell portu markup'ı enjekte edip bu CSS alt-bloğunu atlarsa defekt console/taşma VERMEZ, salt görsel bozulur. Kontrol: (a) tüm marka `--bs-c` token'ları tanımlı mı — **özellikle `--c-mint-deep:#3BB77E`** (Diet pill'ini besler; tanımsızsa `.bs-diet` `--bs-c` geçersiz → Diet pill diğer 4 pill'den farklı render, "buton gibi"); (b) `.bd` computed `font-weight=800` + `GilroyXB`, `.sf` computed `300` + `GilroyLt` (lockup "Dada" kalın / "Gastro" ince ayrımı için — markup doğru olsa bile CSS yoksa ikisi de `.brand-word` Medium'una düşer, düz-ağırlık); (c) `GilroyLight`/`ExtraBold` `@font-face` tanımlı. ⚠️ Wave 4'te bu alt-blok 8 sayfaya tutarsız enjekte edilmişti (4'te tümü, 4'te sadece token). Duplicate ekleme; eksik olanı kanonik `tarif-liste-v1` değerleriyle eşitle.
